...
class TreedataList extends Component {
  ...
  componentWillMount() {
    this.props.navigation.addListener("willFocus", async () => { 
      ...
      // Load list
      this.props.actionsTreedata.loadTreedataList();
    });
  }
  ...
  deleteRow(secId, rowId, rowMap, data) {
    let that = this;
    this.props.actionsTreedata.deleteTreedata(data._id).then(() => {
    ...
    return (
      <Container>
        <Header>
          ...
          <Body>
            <Title>List Treedata</Title>
          </Body>
          <Right>
            <Button
              transparent
              name="menu"
              onPress={() => this.props.navigation.navigate("TreedataEdit")}
            >
              <Text>Add</Text>
            </Button>
          </Right>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ListItem
                noIndent
                onPress={() =>
                  this.props.navigation.navigate("TreedataEdit", {
                    id: item._id
                  })
                }
              >
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.TreedataListReducer.listTreedata
  };
}
