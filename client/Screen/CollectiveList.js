...
class CollectiveList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      list: props.list,
  ...
  deleteRow(secId, rowId, rowMap, data) {
    let that = this;
    this.props.actionsCollective.deleteCollective(data._id).then(() => {
    ...
    return (
      <Container>
        <Header>
          ...
          <Body>
            <Title>List Collective</Title>
          </Body>
          <Right>
            <Button
              transparent
              name="menu"
              onPress={() => this.props.navigation.navigate("CollectiveEdit")}
            >
              <Text>Add</Text>
            </Button>
          </Right>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ListItem
                noIndent
                onPress={() =>
                  this.props.navigation.navigate("CollectiveEdit", {
                    id: item._id
                  })
                }
              >
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.CollectiveListReducer.listCollective
  };
}
